...
class generalRoutes {
  ...
  config(): void {
    ...
    this.routes.get("/view", async (req: Request, res: Response) => {

      const { data: series, error } = await connection.from("Series").select("*, Seasons(*)");
    ...
    this.routes.get("/", async (req: Request, res: Response) => {
      ...
      const { data: seriesNo, error: errorNo } = await connection.from("Series").select("*").eq("visible", false);
